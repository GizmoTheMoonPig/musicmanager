...
public final class MusicToast implements Toast {
	...
	public Visibility render(PoseStack stack, ToastComponent component, long startTime) {
		...
		if (this.width() == 160 && this.getDescription().size() <= 1) {
			...
			this.renderBackgroundRow(stack, 0, 0, 28);

			for (int n = 28; n < this.height() - m; n += 10) {
				this.renderBackgroundRow(stack, 16, n, Math.min(16, this.height() - n - m));
			}

			this.renderBackgroundRow(stack, 32 - m, this.height() - m, m);
		}
		...
		component.getMinecraft().getItemRenderer().renderGuiItem(stack, this.getIcon(), 9, (this.height() / 2) - (16 / 2));
	...
	private void renderBackgroundRow(PoseStack stack, int vOffset, int y, int vHeight) {
		...
		for (int o = uWidth; o < this.width() - n; o += 64) {
			GuiComponent.blit(stack, o, y, 32, vOffset, Math.min(64, this.width() - o - n), vHeight);
		}

		GuiComponent.blit(stack, this.width() - n, y, 160 - n, vOffset, n, vHeight);
	}
